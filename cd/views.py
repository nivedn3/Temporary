...
def  customer2seller(request):
	...
	if request.method=='POST':
		...
		dbobject2=request_conf(Ser_product=pname,Ser_price=price,Ser_image=img,cus_category=cat,Cus_expiry=exptime,cus_loc=cus_loc,Cus_name=name)
		...
		for i in gcmida:
			...
			pson={'delay_while_idle': True, 'collapse_key': 'score_update', 'time_to_live': 108, 'data': {'quoted':"0",'price': price,'cus_loc':cus_loc, 'name': pname,'buyer_name':name,'imgurl': img,'time':exptime,'id':randint(0,10000)}, 'registration_ids': [gcmida[k]]}
...
def s2c(request):
		if request.method=='POST':
				...
				if k:
					...
					dbobject2=selldb(Cus_id=cid,Q_price=qprice,Sel_type=prtype,Sel_comments=comment,Sel_id=Sel_id,Sel_deltype=deltype)
					dbobject2.save()
